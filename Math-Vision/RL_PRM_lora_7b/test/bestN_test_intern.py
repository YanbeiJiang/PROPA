...
def generate_greedy(prompt, image_path, gpu_id):
    ...
    while attempts < 5:  # Retry up to 5 times
        try:
            ...
            request_config = RequestConfig(max_tokens=256, seed=42, n=4, temperature=1.0)
...
def reward_model(prompt, image_path):
    ...
    while attempts < 5:  # Retry up to 5 times
        try:
            engine = InferClient(host='0.0.0.0', port=port_num)
            request_config = RequestConfig(seed=42, n=1, temperature=0)
            
            infer_requests = [
                InferRequest(messages=[{"role": "user", "content": "<image>"+prompt}], images=[image_path])
            ]
            resp_list = engine.infer(infer_requests, request_config)
...
parser.add_argument("--input", type=str, default="/data/projects/punim1996/Data/AVR-RL/Math-Vision/Math_Vision_test.json", help="input")
parser.add_argument("--output", type=str, default="/data/gpfs/projects/punim1996/Data/AVR-RL/Math-Vision/RL_PRM/test/concatenate_inference_results_intern_without_training.json", help="output")
