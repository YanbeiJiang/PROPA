...
for batch_num in $(seq 0 $((max_batch_num_test-1))); do
    python MCTS_test_intern.py --epoch $best_epoch_num --batch_num $batch_num --batch_size 16 --input '../../Math_Vision_test.json' --output_base '../tree_data_intern_test'
done
echo "In Domain results: "
python find_best_path.py --epoch $best_epoch_num --dataset '../../Math_Vision_test.json' --tree_path '../tree_data_intern_test'

max_batch_num_test=35

for batch_num in $(seq 0 $((max_batch_num_test-1))); do
    python MCTS_test_intern.py --epoch $best_epoch_num --batch_num $batch_num --batch_size 20 --input '../../DynaMath_test.json' --output_base '../tree_data_intern_test_OOD'
done

echo "Out of Domain results: "
python find_best_path.py --epoch $best_epoch_num --dataset '../../DynaMath_test.json' --tree_path '../tree_data_intern_test_OOD'
