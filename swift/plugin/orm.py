...
class custom_grpo(ORM):
    def __call__(self, completions, solution, **kwargs) -> List[float]:
        ...
        # for i, completion in enumerate(completions):
        #     print(f"completions{i}: {completion}")
        # print("--------------------")
        ...
        #         print(f"solution{i}: {each}")
        for each in solution[0]:
            rewards.append(float(each.split(" **Label:** ")[1]))
        # print("rewards: ", rewards)
        #print("len rewards: ", len(rewards))
        #assert(len(rewards)==4)
        return rewards

class custom_grpo2(ORM):
    def __call__(self, completions, solution, **kwargs) -> List[float]:
        rewards = []
        # for i, completion in enumerate(completions):
        #     rewards.append(1.0)
...
class external_r1v_acc(ORM):

    def __call__(self, completions, solution, **kwargs) -> List[float]:
        rewards = []
        for content, sol in zip(completions, solution):
            match_generated = re.search(r"<answer>(\d+)</answer>", content)
            match_gt = re.search(r"<answer>(\d+)</answer>", sol)

            if match_generated and match_gt and match_generated.group(1) == match_gt.group(1):
                rewards.append(1.0)
            else:
                rewards.append(0.0)
        #print(rewards)
...
orms["custom_grpo"] = custom_grpo
orms["custom_grpo2"] = custom_grpo2
orms["external_r1v_acc"] = external_r1v_acc