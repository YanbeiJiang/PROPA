...
class GRPOTrainer(RLHFTrainerMixin, SwiftMixin, HFGRPOTrainer):
    ...
    def __init__(self,
        ...
        if self.args.dynamic_sample:
            #print("self.args.dynamic_sample: ", self.args.dynamic_sample)
    ...
    def _infer_multi_turn(self, inputs_slice: np.ndarray, request_config: RequestConfig) -> Union[OutputsType, List]:
        ...
        results: List[ChatCompletionResponse] = self._engine_infer_mock(
            infer_requests=inputs_slice, request_config=request_config, use_tqdm=False)
        #print(len(inputs_slice))
        #print(inputs_slice)
        #print(results)
        ...
        if self.multi_turn_func:
            ...
            while len(inputs_slice) > 0:
                ...
                if len(inputs_slice) > 0:
                    ...
                    results = self._engine_infer_mock(
            ...
            for i, output in enumerate(results):
                ...
                # print("input: ", inputs_slice)
                # print("len input: ", len(inputs_slice))
                for choice in output.choices:
                    _input: Dict = deepcopy(inputs_slice[i])
                    InferRequest.remove_response(_input['messages'])
                    _input['messages'].append({'role': 'assistant', 'content': choice.message.content})
    ...
    def _generate_completions(self, inputs: InputsType) -> InputsType:
        ...
        """
        #print("self.args.max_resample_times: ", self.args.max_resample_times)
        mode = 'train' if self.model.training else 'eval'
        if self.use_fast_infer:
    ...
    def _engine_infer(
        ...
        *,
        ...
        #print(len(infer_requests))
        print(infer_requests)
        # for infer_request in infer_requests:
        #     print("----------")
        #     print(infer_request["messages"][1]["content"])
        #print(self.engine.infer(infer_requests, request_config, use_tqdm=use_tqdm))
        if self.is_external_vllm:
    ...
    def _engine_infer_mock(
        ...
        # Option 1: Create mock responses using simple objects/dicts
        # mock_responses = []
        # for i, request in enumerate(infer_requests):
        #     # Create mock response using basic Python objects
        ...
        #     mock_message.content = f'Mock response Mock response Mock response for request {i}'
        ...
        # split_responses = []
        # for i in range(0, len(infer_requests), self.num_generations):
        #     if self.is_external_vllm:
        #         self._process_infer_requests_images(infer_requests)
        #         original_response = self.vllm_client.infer(infer_requests.tolist()[i:i+self.num_generations], asdict(request_config), use_tqdm=use_tqdm)
        #     else:
        #         original_response = self.engine.infer(infer_requests[i:i+self.num_generations], request_config, use_tqdm=use_tqdm)

        #     for choice in original_response[0].choices:
        #         # 深拷贝原始响应以保留其他字段
        #         new_response = deepcopy(original_response[0])
        #         # 只保留当前 choice
        #         new_response.choices = [choice]
        #         split_responses.append(new_response)
        # return split_responses

        response_list: List[ChatCompletionResponse] = []
        for i in range(0, len(infer_requests), self.num_generations):
            infer_request = infer_requests[i]
            content_list = infer_request["solution"]
            #print("content_list", len(content_list))
            for idx, content in enumerate(content_list):
                content = content.split(" **Label:**")[0]
                msg = ChatMessage(role='assistant', content=content)
                choice = ChatCompletionResponseChoice(index=0, message=msg, finish_reason='stop')
                response = ChatCompletionResponse(
                    id=None,
                    object="chat.completion",
                    created=int(time.time()),
                    model="Qwen2.5-VL-3B-Instruct",  # 可根据你的模型实际修改
                    choices=[choice],
                    usage=None  # 你也可以构造 UsageInfo(...)，可选
                )
                response_list.append(response)
        # print("infer_requests: ", len(infer_requests))
        # print("num_generations: ", self.num_generations)
        # print(len(response_list))
        return response_list
        #print(request_config)
        # engine = InferClient(host='0.0.0.0', port=11000)
        # if infer_requests[0]["model"] == "Qwen/Qwen2.5-VL-3B-Instruct":
        #     request_config = RequestConfig(max_tokens=128, seed=42, n=4, temperature=1.0)
        # else:
        #     request_config = RequestConfig(max_tokens=128, seed=42, n=4, temperature=0, num_beams=4)
        # split_responses = []
        # for i in range(0, len(infer_requests), self.num_generations):
        #     infer_request = [
        #             InferRequest(messages=[{'role': 'system', 'content': "A conversation between User and Assistant. The input by user may include some existing steps to solve the question and the Assistant should continue to derive only the next step based on these existing steps. If the input does not provide any existing steps, the Assistant need to analyze the problem and then give the first step in solving the problem. If the Assistant think it has reached the final step, provide the final answer number following the format 'The final answer is ...', otherwise continue to the next inference step."}, {"role": "user", "content": infer_requests[i]["messages"][1]["content"]}], images=infer_requests[i]["images"])
        #         ]
        #     original_response = engine.infer(infer_request, request_config)
