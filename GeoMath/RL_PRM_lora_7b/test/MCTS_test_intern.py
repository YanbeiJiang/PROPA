...
class MCTS:
    ...
    def expand(self, node: MCTSNode, question: str, image_path: str, gpu_id, epoch):
    ...
    def reward_model(self, node, question, image_path, text, gpu_id):
        ...
        while attempts < 5:  # Retry up to 5 times
            try:
                ...
                infer_requests = [
                    InferRequest(messages=[{"role": "user", "content": "<image>"+f"{question} {self.get_trajectory(node)} {text}"}], images=[image_path])
                ]
                resp_list = engine.infer(infer_requests, request_config)
    ...
    def check_if_final_answer(self, text: str) -> bool:
        ...
        if "The correct answer is" in text:
            text = text.replace("The correct answer is", "The final answer is")
    ...
    def search(self, question: str, gpu_id, num_iterations: int = 10, image_path: str = None, epoch = 0) -> List[MCTSNode]:
        ...
        i = 0
        while i < num_iterations or self.final_count < 3:
            # Selection
            print("ITERATIONS: ", i)
            ...
            if not leaf.is_terminal() and not leaf.is_fully_expanded():
                children = self.expand(leaf, question, image_path, gpu_id, epoch)
...
def mcts_main(args_dict):
    ...
    #output = f'/data/projects/punim1996/Data/AVR-RL/GeoMath/RL_PRM_lora_7b/tree_data_intern_test_OOD/tree_data_intern_epoch{epoch}_{start_index}.json'
...
if __name__ == '__main__':
    ...
    parser.add_argument("--input", type=str, default='/data/projects/punim1996/Data/AVR-RL/GeoMath/Math_Vision_test.json', help="checkpoint")
    parser.add_argument("--output_base", type=str, default='/data/projects/punim1996/Data/AVR-RL/GeoMath/RL_PRM_lora_7b/tree_data_intern_test', help="checkpoint")
