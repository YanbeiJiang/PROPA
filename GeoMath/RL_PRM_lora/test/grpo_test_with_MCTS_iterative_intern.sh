...
for epoch in $(seq 3 7); do
    ...
    for batch_num in $(seq 0 $((max_batch_num_test-1))); do
        python MCTS_test_intern.py --epoch $epoch_num_prev --batch_num $batch_num --batch_size 20 --input '../../GeoMath_val.json' --output_base $val_result_dir
    done

    #python find_best_path.py --epoch $epoch_num_prev --dataset '../../GeoMath_val.json' --tree_path $val_result_dir


    accuracy_output=$(python find_best_path.py --epoch $epoch_num_prev --dataset '../../GeoMath_val.json' --tree_path $val_result_dir)
...
if [ -d "$base_output_dir_policy" ]; then
    ...
    echo "Error: Base output directory $base_output_dir_policy does not exist"
    exit 1
# best_checkpoint="../output_grpo/output_grpo_epoch6_24/v1-20251004-233457/checkpoint-2"
# best_checkpoint_rm="./output_rm_grpo/output_rm_epoch6/v1-20251013-083528/checkpoint-12036"# fi
...
max_batch_num_test=41

for batch_num in $(seq 0 $((max_batch_num_test-1))); do
    python MCTS_test_intern.py --epoch $best_epoch_num --batch_num $batch_num --batch_size 20 --input '../../GeoMath_test.json' --output_base '../tree_data_intern_test'
done
echo "In Domain results: "
python find_best_path.py --epoch $best_epoch_num --dataset '../../GeoMath_test.json' --tree_path '../tree_data_intern_test'

max_batch_num_test=40

for batch_num in $(seq 0 $((max_batch_num_test-1))); do
    python MCTS_test_intern.py --epoch $best_epoch_num --batch_num $batch_num --batch_size 20 --input '../../Geometry_test.json' --output_base '../tree_data_intern_test_OOD'
done

echo "Out of Domain results: "
python find_best_path.py --epoch $best_epoch_num --dataset '../../Geometry_test.json' --tree_path '../tree_data_intern_test_OOD'
