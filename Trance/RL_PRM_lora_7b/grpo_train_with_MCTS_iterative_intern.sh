...
# Main epoch loop
for epoch in $(seq 5 $((max_epoch-1))); do
    echo "Starting epoch $epoch"
    # Inner batch loop
    for batch_num in $(seq 10 $((max_batch_num-1))); do
        ...
        PORTS=(40000 40001 40002 40003 40004 50000 50001 50002 50003)
        ...
        # if [ "$epoch" -ne 0 ]; then
        #     ./deploy_vllm_online_intern_rm.sh
        # fi
        ...
        #python save_checkpoints_intern.py --checkpoint $checkpoint_dir
        # Delete all contents in the destination directory
        rm -rf "./checkpoints_grpo/intern_checkpoint/policy_model/"*

        # Copy all contents from the source directory to the destination directory
        cp -r "${checkpoint_dir}/"* "./checkpoints_grpo/intern_checkpoint/policy_model"
        #python save_checkpoints_intern.py --checkpoint $checkpoint_dir

        ./deploy_vllm_online_intern_policy_grpo.sh

        sleep 60

        for i in {1..5}; do
            echo "尝试第 $i 次..."
            if python run_MCTS_train_intern_grpo.py --epoch $epoch --batch_num $batch_num --batch_size $batch_size; then
                echo "执行成功！"
                break
            else
                echo "第 $i 次执行失败"
        ...
        if [ $? -ne 0 ]; then
            echo "Error: run_MCTS_train_intern.py failed for epoch $epoch, batch $batch_num"
            exit 1
        fi
        
        # Step 2: Run create_GRPO_train_intern.py
        echo "Step 2: Running create_GRPO_train_intern.py"
        python create_GRPO_train_intern.py --epoch $epoch --batch_num $batch_num --batch_size $batch_size
        if [ $? -ne 0 ]; then
            echo "Error: create_GRPO_train_intern.py failed for epoch $epoch, batch $batch_num"
            exit 1
        fi
        ...
        json_file="./grpo_data_intern/grpo_data_intern_epoch${epoch}_${batch_num}.json"
        skip_grpo=false
        if [ -s "$json_file" ] && [ "$(jq 'length' "$json_file")" -gt 0 ]; then
            ...
            swift rlhf \
                ...
                --dataset "./grpo_data_intern/grpo_data_intern_epoch${epoch}_${batch_num}.json" \
        ...
        python create_SFT_train_intern_after_grpo.py --epoch $epoch --batch_num $batch_num
        
        json_file="./SFT_after_grpo/SFT_data_intern_epoch${epoch}_${batch_num}.json"
        if [ -f "$json_file" ]; then
            ...
            swift sft \
                ...
                --resume_from_checkpoint "$checkpoint_dir" \
                --resume_only_model true \
                --dataset "./SFT_after_grpo/SFT_data_intern_epoch${epoch}_${batch_num}.json" \
                --split_dataset_ratio 0 \
                --train_type lora \
                --lora_rank 8 \
                --lora_alpha 32 \
