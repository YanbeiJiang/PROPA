...
class MCTS:
    ...
    def generate_responses(self, prompt, image_path, port_num):
        ...
        # request_config = RequestConfig(max_tokens=256, seed=42, n=4, temperature=1.2, top_k=50, top_p=0.95)
        ...
        # return responses
        request_config = RequestConfig(max_tokens=256, seed=None, n=3, temperature=1.2, top_k=50, top_p=0.95)
        ...
        infer_requests = [
            InferRequest(messages=[
                {'role': 'system', 'content': (
                    ...
                    "The input by user may include some existing steps to solve the question and the Assistant "
                    "should continue to derive only the next step based on these existing steps. "
                    "If the input does not provide any existing steps, the Assistant need to analyze the problem "
                    "and then give the first step in solving the problem. "
                    "If the Assistant think it has reached the final step, provide the final answer number "
                    "following the format 'The final answer is ...', otherwise continue to the next inference step."
                )}, 
                {"role": "user", "content": "<image><image>" + prompt}
            ], images=image_path)
        ]
    ...
    def generate_greedy(self, prompt, image_path, port_num):
        ...
        engine = InferClient(host='0.0.0.0', port=port_num)
        request_config = RequestConfig(max_tokens=256, seed=42, n=1, temperature=0)
    ...
    def expand(self, node: MCTSNode, question: str, image_path: str, port_num, epoch):
        ...
        for generated_text in outputs:
            is_final_answer, reward = self.check_if_final_answer(generated_text)
    ...
    def search(self, question: str, gpu_id, num_iterations: int = 10, image_path: str = None, epoch = 0) -> List[MCTSNode]:
        ...
        for i in range(num_iterations):
            ...
            if not leaf.is_terminal() and not leaf.is_fully_expanded():
                ...
                #available_ports = [40000, 40001, 40002, 40004, 50000, 50001, 50002, 50003]
                available_ports = [40000, 40001, 40002, 40004]
                #available_ports = [40000, 40001]
                # 首先根据gpu_id选择初始端口
                if gpu_id == 0:
                    initial_port = 40000
                elif gpu_id == 1:
...
def process_dataset(input_file: str, output_file: str,
    ...
    for item in tqdm(dataset[start_index:end_index], desc="Processing examples"):
        ...
        #     question = "Think step by step to solve the problem in the image: " + messages[0]["content"].split("<image><image>")[1].split(" Please ONLY respond with a number.")[0].strip() + " The final answer should be a number. Please ONLY response one step at a time."
        # else:
        #     question = "Think step by step to solve the problem in the image: " + messages[0]["content"].split("<image><image>")[1].split(" Please ONLY respond with the selected option letter.")[0].strip() + " The final answer should be a option letter. Please ONLY response one step at a time."
...
def mcts_main(args_dict):
    ...
    output = f'./tree_data_intern_grpo/tree_data_intern_epoch{epoch}_{start_index}.json'
    # if os.path.exists(output):
    #     print(f"File {output} already exists. Exiting.")
    #     return
    print(f"run from {start_index} to {end_index}")
    process_dataset(
        input_file = '../Trance_train.json',
        output_file = output,
        num_iterations=20,
