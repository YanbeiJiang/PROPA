...
def generate_greedy(prompt, image_path, gpu_id):
    ...
    while attempts < 5:  # Retry up to 5 times
        try:
            ...
            max_reward = -1.0
            for i in range(2):
                request_config = RequestConfig(max_tokens=256, seed=42, n=2, temperature=1.0)
...
for item in tqdm(dataset, desc="Processing examples"):
    ...
    all_steps = []
    max_steps = 8  # Maximum number of steps to prevent infinite loops
    step_count = 0
