...
output_dir="$1"
val_dataset='../Trance_val_ORM.json'
...
result_path="./result_rl/$model_name/infer_result/Long_SFT.jsonl"
checkpoint_path="../Long_SFT/output_SFT_RL/InternVL2_5-8B/v0-20251013-091302/checkpoint-1000"
...
swift infer \
    --adapters "$checkpoint_path" \
    --infer_backend pt \
    --val_dataset '../Trance_test_ORM.json' \
    --temperature 0 \
    --max_new_tokens 4096 \
    --use_hf true \
    --max_batch_size 1 \
...
result_path="./result_rl/$model_name/infer_result/sft_OOD1.jsonl"
if [ -f "$result_path" ]; then
    echo "文件存在，正在删除..."
    rm "$result_path"
    echo "文件已删除: $result_path"
...
swift infer \
    --adapters "$checkpoint_path" \
    --infer_backend pt \
    --val_dataset '../TranceL_test_ORM.json' \
    --temperature 0 \
    --max_new_tokens 4096 \
    --use_hf true \
    --max_batch_size 1 \
...
result_path="./result_rl/$model_name/infer_result/sft_OOD2.jsonl"
if [ -f "$result_path" ]; then
    echo "文件存在，正在删除..."
    rm "$result_path"
    echo "文件已删除: $result_path"
...
swift infer \
    --adapters "$checkpoint_path" \
    --infer_backend pt \
    --val_dataset '../TranceR_test_ORM.json' \
    --temperature 0 \
    --max_new_tokens 4096 \
    --use_hf true \
    --max_batch_size 1 \
